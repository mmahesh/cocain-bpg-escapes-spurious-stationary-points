...
Convex-Concave Backtracking for Inertial Bregman 
Proximal Gradient Algorithms in Non-Convex Optimization
...
# here i used 1e-3, but actually any value >0 is fine
uL_est = 1/(1-del_val) + 1e-3

# It is not preferable to set a large upper L as it may result in very small steps.
...
parser = argparse.ArgumentParser(
	description='Experiments: CoCaIn BPG escapes Spurious Stationary Points')
parser.add_argument('--init_Ux', '--init_Ux', default=0,
					type=int,  dest='init_Ux')  # x coordinate
parser.add_argument('--init_Uy', '--init_Uy', default=0,
					type=int,  dest='init_Uy')  # y coordinate
...
def get_proj_log(u, l):
	...
	if ((np.abs(u) - 1)**2 - 4*(l-np.abs(u))) >= 0:
		new_x = [0, 0.5*((np.abs(u)-1) \
							+ np.sqrt(((np.abs(u) - 1)**2 - 4*(l-np.abs(u))))),
				 0.5*((np.abs(u)-1) \
					 		- np.sqrt(((np.abs(u) - 1)**2 - 4*(l-np.abs(u)))))]
		new_y = [0, 0, 0]
		for i in [0, 1, 2]:
...
def make_update(y, grad, uL_est):
	...
	return np.array([get_proj_log(temp_p[0], (1/uL_est)),\
						 get_proj_log(temp_p[1], (1/uL_est))])
...
def find_gamma(U, prev_U, uL_est, lL_est):
	...
	# Inertial step coefficient
	kappa = (del_val - eps_val)*(uL_est/(uL_est+lL_est))
	y_U = U + gamma*(U-prev_U)  # extrapolation
	while (kappa*breg(prev_U, U, breg_num=breg_num) \
				< breg(U, y_U, breg_num=breg_num)):
		# in the above condition we can also 
		# threshold rather than absolute condition
...
for i in range(max_iter):
	...
	lyapunov_vals = lyapunov_vals + \
		[(1/uL_est)*temp + del_val*breg(U, prev_U, breg_num=breg_num)]
