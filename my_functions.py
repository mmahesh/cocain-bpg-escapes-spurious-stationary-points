...
def main_func( U, fun_num=1):
	# function value
	# fun_num is not necessary TODO: will fix this later but the code works 
	if fun_num==1:
		temp_sum = 0
		count = 0
		x = U[0]
		...
		return 0.5*np.log(1+100*(np.abs(x-1)**2))\
				+0.5*np.log(1+(100*np.abs(y-1)**2))\
...
def abs_func( U, U1, abs_fun_num=1, fun_num=1):
	...
	if fun_num==1 and abs_fun_num==1:
		...
		return np.log(1+np.abs(x))+np.log(1+np.abs(y))\
				 	+ np.sum(np.multiply(G,U-U1))\
		 			+ 0.5*np.log(1+100*(np.abs(x_1-1)**2))\
					+ 0.5*np.log(1+(100*np.abs(y_1-1)**2))
